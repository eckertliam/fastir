...
pub fn extract_inline_features(bc: Bound<PyBytes>) -> PyResult<Vec<u8>> {
    ...
    let callsite_intersections: HashSet<&(String, String, String)> = mod_features.call_sites.intersection(&inlined_mod_features.call_sites).collect();
    ...
    for (caller_name, bb_name, callee_name) in mod_features.call_sites.clone() {
        let callee_features = mod_features.fn_feats.get(&callee_name).ok_or(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!("Callee function {} not found", callee_name)))?;
        let caller_features = mod_features.fn_feats.get(&caller_name).ok_or(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!("Caller function {} not found", caller_name)))?;
        ...
        caller_to_callee_instr_ratio_vec.push(caller_features.instruction_count as f64 / callee_features.instruction_count as f64);
        bb_name_vec.push(bb_name.clone());
        llvm_inlining_decision_vec.push(callsite_intersections.contains(&(caller_name, bb_name, callee_name)));
    }
